...
import '../../model/car/car_model.dart' as car; // Aliased import
import '../../model/global/response_model/response_model.dart';
import '../../model/room_model/RoomModel.dart';
import '../../repo/car/car_repo.dart';
...
class CarController extends GetxController {
  ...
  int numberOfGuests = 12; //all place
  int numberOfRooms = 12; //all place
  ...
  List<car.Ad> adsList = []; // Using alias
  List<car.Owner> popularCarList = []; // Using alias
  List<car.Owner> featuredCarList = []; // Using alias
  List<home.PopularCity> popularCityList = []; // Using alias
  int totalPopularCar = -1;
  ...
  Future<void> loadCarData() async {
    ResponseModel model = await carRepo.getData();
    if (model.statusCode == 200) {
      car.CarResponseModel carResponseModel = car.CarResponseModel.fromJson(
          jsonDecode(model.responseJson)); // Using alias

      if (carResponseModel.data != null) {
        totalPopularCar =
            int.tryParse(carResponseModel.data?.totalPopularCars ?? '-1') ?? -1;
        totalPopularCities =
            int.tryParse(carResponseModel.data?.totalPopularCities ?? '-1') ??
                -1;
        totalFeaturedOwner =
            int.tryParse(carResponseModel.data?.totalFeaturedOwners ?? '-1') ??
                -1;
      }

      if (carResponseModel.data!.ads != null &&
          carResponseModel.data!.ads!.isNotEmpty) {
        adsList.clear();
        adsList.addAll(carResponseModel.data!.ads!);
      }

      if (carResponseModel.data?.owners != null &&
          carResponseModel.data!.owners!.isNotEmpty) {
        popularCarList.clear();
        popularCarList.addAll(carResponseModel.data!.owners!);
      }

      if (carResponseModel.data?.popularCities != null &&
          carResponseModel.data!.popularCities!.isNotEmpty) {
        popularCityList.clear();
        popularCityList.addAll(carResponseModel.data!.popularCities!
            as Iterable<home.PopularCity>);
      }

      if (carResponseModel.data!.featuredOwners != null &&
          carResponseModel.data!.featuredOwners!.isNotEmpty) {
        featuredCarList.clear();
        featuredCarList.addAll(carResponseModel.data!.featuredOwners!);
      }
    } else {
  ...
  Future<void> loadSearchDestinationData() async {
    ...
    ResponseModel model =
        await carRepo.getSearchDestinationData(searchKeyword, '$page');
    if (model.statusCode == 200) {
