...
class AddNewDepositController extends GetxController{
  ...
  MethodElement? paymentMethod   = MethodElement(name: MyStrings.selectOne,id: -1);
  ...
  setPaymentMethod(MethodElement? method) {
  ...
  Future<void> getDepositMethod(String bookingId) async {
    ...
    ResponseModel responseModel = await depositRepo.getDepositMethods(bookingId);
    ...
    if (responseModel.statusCode == 200) {
      ...
      if (methodsModel.message != null && methodsModel.message!.success != null) {
        ...
        if(methodsModel.data?.booking != null){
          amountController.text =  Converter.formatNumber(methodsModel.data!.booking!.dueAmount!);
          this.bookingId = methodsModel.data!.booking!.id!;
        }

        List<MethodElement>? tempList = methodsModel.data?.methods;
  ...
  Future<void> submitDueAmount() async {
