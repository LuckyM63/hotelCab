...
class _PopularHotelScreenState extends State<PopularHotelScreen> {
  ...
  void initState() {
    ...
    final controller = Get.put(PopularHotelController(popularHotelRepo: Get.find()));
          ...
          body: controller.isLoading ? const CustomLoader() : SafeArea(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                var hotelList = controller.popularHotelList[index].hotelSetting;

                return GestureDetector(
                  onTap: () {
                    Get.toNamed(RouteHelper.hotelDetailsScreen,arguments: hotelList?.ownerId.toString() ?? '-1');
                  },
                  child: Container(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            Expanded(child: Text(hotelList?.address?.tr ?? '',style: regularDefault.copyWith(color: MyColor.primaryTextColor),maxLines: 2,overflow: TextOverflow.ellipsis,))
                          ],
                        ),
                        const SizedBox(height: Dimensions.space8,),
                        Row(
                          children: [
                            Text("${hotelList?.starRating ?? ''} ${MyStrings.starHotel.tr}",style: regularDefault,),
                                ...
                                child: RichText(
                                  text: TextSpan(
                                    ...
                                    children: <TextSpan>[
                                      ...
                                      TextSpan(
                                          text: " ${controller.popularHotelRepo.apiClient.getCurrencyOrUsername(isSymbol: true)}${Converter.formatNumber(controller.popularHotelList[index].minimumFare?? '-1',precision: 0)}",
                                          style: semiBoldDefault
                                      ),
                                      TextSpan(
                                          text: '/${MyStrings.perNight.tr}',
                                          style: regularDefault.copyWith(color: MyColor.bodyTextColor,overflow: TextOverflow.ellipsis)
                                      ),
